<template>
	<view>
		<view class="upload">
			...
			<u-circle-progress v-if='circle' type="primary upload" :percent="progress" :duration="getduration(duration)" bg-color='#ffffffd6'>
	...
	export default{
		props: {
			...
			clearIcon: {
				...
				default:'../../static/images/cols.png'
			},
			uploadIcon: {
				type: String,
				default: ''
						...
						uni.chooseImage({
							...
							success: (res) => {
								...
								for(let i = 0; i< res.tempFiles.length; i++){
									if(Math.ceil(res.tempFiles[i].size / 1024) < this.upload_image_max * 1024){
										...
										uni.showModal({
											title: '提示',
											content: `图片超出限制${this.upload_image_max}MB`
										});
									}
								}
							},
						...
						uni.chooseVideo({
							...
							success: (res) => {
								if(Math.ceil(res.size / 1024) < this.upload_max * 1024){
									...
									uni.showModal({
										title: '提示',
										content: `视频超出限制${this.upload_max}MB`
									});
								}
							},
							fail: (err) => {
