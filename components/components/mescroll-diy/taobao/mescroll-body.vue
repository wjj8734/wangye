<template>
	<view 
		...
		<view class="mescroll-body-content mescroll-wxs-content" :style="{ transform: translateY, transition: transition }" :change:prop="wxsBiz.callObserver" :prop="callProp">
			...
			<!-- <mescroll-down :option="mescroll.optDown" :type="downLoadType" :rate="downRate" :is-show-moon="isShowMoon"></mescroll-down> -->
			<view v-if="mescroll.optDown.use" class="mescroll-downwarp" :class="{'moon-show':isShowMoon}">
				<view class="downwarp-content">
					<view v-if="isShowDownLoad" class="downwarp-progress" :class="{'mescroll-rotate':isDownLoading}">
						<view class="progress-arc">
							<view class="progress-left-arc wxs-left-arc" :style="{'transform':rotateLeft}"></view>
						</view>
						<view class="progress-arc">
							<view class="progress-right-arc wxs-right-arc" :style="{'transform':rotateRight}"></view>
						</view>
					</view>
		...
		<!-- 淘宝二楼 -->
		<view class="mescorll-erlou" :class="{'mescorll-erlou-in':isShowErlou, 'mescorll-erlou-out':isShowErlou===false}" @click="closeErlou">
			<!-- 此处可以加任何您想加的东西 -->
		</view>
	</view>
</template>
	...
	export default {
		...
		data() {
			return {
				...
				isShowErlou: null, //下拉刷新: 是否显示淘宝二楼 默认null 不能为false
				isShowMoon: false, //下拉刷新: 是否显示月亮的动画
			};
		},
		props: {
		...
		computed: {
			...
			isShowDownLoad() {
				return this.downRate < 2
			},
			// 左半圆旋转的角度
			rotateLeft(){
				if (this.downRate<1) {
					let progress = 360 * this.downRate;
					if (progress<180) {
						return 'rotate(45deg)';
					} else{
		...
		methods: {
			...
			// 关闭二楼
			closeErlou(){
				if(!this.isShowMoon) this.isShowErlou=false
			},
			// wxs的isLockCallback回调
			wxsCallLock(msg){
				this.downRate = msg.rate
				this.mescroll.isLockCallback = msg.isLockCallback
			}
		},
			...
			let diyOption = {
				...
				down: {
					...
					onMoving(mescroll, rate, downHight) {
						...
						mescroll.isLockCallback = (rate >= 2); //大于2倍offset,自定义下拉刷新,进入淘宝二楼
					},
					beforeLoading: function(mescroll) {
						...
						if (mescroll.isLockCallback) {
							//月亮动画 (2s)
							vm.isShowMoon = true;
							vm.downHight = 2 * mescroll.getBodyHeight(); // 往下拉
							setTimeout(function(){ // 往回收
								vm.downHight = 0.5 * mescroll.getBodyHeight();
							},1000)
							setTimeout(function(){ // 结束
								vm.isShowMoon = false;
							...
							vm.isShowErlou = true
							// 触发wxs的方法 (值改变才触发更新)
							vm.callProp = {callType: "showErlou", t: Date.now()} 
						}
						//如果要完全自定义下拉刷新,那么return true,此时将不再执行showLoading(),callback();
						return mescroll.isLockCallback;
					},
...
<style>
	...
	.mescorll-erlou{
		...
		right: 0;
		bottom: 0;
		background-image: url(https://www.mescroll.com/img/taobao/mescorll-erlou.jpg);
	...
	.mescorll-erlou-in{
		pointer-events: auto;
		animation: erlouIn 1.5s linear forwards;
	}
	@keyframes erlouIn {
		0% {
			opacity: 0;
	...
	.mescorll-erlou-out{
		pointer-events: none;
		animation: erlouOut 1.5s linear forwards;
	}
	@keyframes erlouOut {
		0% {
			opacity: 1;
